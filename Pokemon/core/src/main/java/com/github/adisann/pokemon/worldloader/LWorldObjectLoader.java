...
public class LWorldObjectLoader
		extends AsynchronousAssetLoader<LWorldObjectDb, LWorldObjectLoader.LWorldObjectDbParameter> {
	...
	public void loadAsync(AssetManager asman, String filename, FileHandle file, LWorldObjectDbParameter parameter) {
		...
		if (root == null || !root.getName().equals("LWorldObjects")) {
			System.err.println("Root node in " + filename + " is " + (root != null ? root.getName() : "null")
					+ " expected LWorldObjects");
			Gdx.app.exit();
		}
		...
		for (int i = 0; i < root.getChildCount(); i++) {
			...
			if (!imageName.getName().equals("imageName")) {
				System.err.println(
						"Found " + imageName.getName() + "-element where expected imageName-element in " + filename);
				Gdx.app.exit();
			}
			...
			if (!size.getName().equals("size")) {
				System.err.println(
						"Found " + imageName.getName() + "-element where expected size-element in " + filename);
				Gdx.app.exit();
			}
			...
			if (!tilesElement.getName().equals("tiles")) {
				System.err.println(
						"Found " + imageName.getName() + "-element where expected tiles-element in " + filename);
				Gdx.app.exit();
			}
